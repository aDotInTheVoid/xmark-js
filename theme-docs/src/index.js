  ...
  hooks: {
    'modules:before': ({ nuxt }) => {
      ...
      nuxt.options.tailwindcss.configPath =
        nuxt.options.tailwindcss.configPath ||
        path.resolve(nuxt.options.rootDir, 'tailwind.config.js')
      nuxt.options.tailwindcss.cssPath =
        nuxt.options.tailwindcss.cssPath ||
        path.resolve(
          nuxt.options.rootDir,
          nuxt.options.dir.assets,
          'css',
          'tailwind.css'
        )
      // Extend `/` route
      ...
      if (nuxt.options.dev) {
        nuxt.options.css.push(
          path.resolve(__dirname, 'assets/css/main.dev.css')
        )
      }
      ...
      nuxt.hook('tailwindcss:config', function (defaultTailwindConfig) {
        Object.assign(
          ...
          defu(defaultTailwindConfig, tailwindConfig({ docsOptions, nuxt }))
        )
      })
    },
  },
...
export default (userConfig) => {
  ...
  if (userConfig.env && userConfig.env.GITHUB_TOKEN) {
    ...
    console.warn(
      '[security] Avoid passing `env.GITHUB_TOKEN` directly in `nuxt.config`. Use `.env` file instead!'
    )
